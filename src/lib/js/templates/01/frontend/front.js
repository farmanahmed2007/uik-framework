...
jQuery(document).ready(function () {
	app_cardealer_app_front = new THEMEMAKERS_APP_CARDEALER_FRONT();
	app_cardealer_app_front.init();

	jQuery("#add_car_video").life('click', function () {
		...
		init: function () {
			jQuery("#set_dealer_loan_rate").click(function () {
				...
				jQuery.post(ajaxurl, data, function (response) {
					...
					show_info_popup(lang_loan_rate_updated);
				});
				return false;
			});
			...
			jQuery(".js_feature_user_car").life('click', function () {
				...
				if (is_checked) {
					var c = confirm(lang_sure_featured);
					if (!c) {
						return false;
					}
				}
				...
				jQuery.post(ajaxurl, data, function (response) {
					...
					if (response) {
						if (response.items) {
							...
							var notice = jQuery('<p class="notice"><i class="icon-info-circled"></i> There\'s no cars by this filter</p>');
						...
						var notice = jQuery('<p class="notice"<i class="icon-info-circled"></i> Please set one or more car features for filtering</p>');
						change_items.append(notice);
					}
				});
			}
			...
			jQuery('.convert').life('click', function () {
				...
				if ($this.attr('class').indexOf('active') != -1) {
					...
					jQuery.post(ajaxurl, data, function (response) {
						...
						if (serving_quota) {
							var converter_value = jQuery('<h6>'+lang_currency_converter+'</h6><p>' + serving_quota + '</p>');
						}
						else {
							var converter_value = jQuery('<h6>'+lang_currency_converter+'</h6><input class="value_holder" value="" type="text" readonly><ul>' + buttons + '</ul>');
						}
			...
			jQuery(".js_car_watch_list").life('click', function () {
				if (can_add_to_watch_list == 0) {
					show_info_popup(lang_cars6);
